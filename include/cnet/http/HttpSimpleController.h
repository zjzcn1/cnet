...
namespace cnet
{
class HttpSimpleControllerBase : public virtual HttpObjectBase
{
  public:
...
template <typename T>
class HttpSimpleController : public HttpObject<T>, public HttpSimpleControllerBase
{
  public:
  ...
  protected:
    ...
    static void __registerSelf(const std::string &path, const std::vector<any> &filtersAndMethods)
    {
        ...
        HttpApp::instance().registerHttpSimpleController(path, HttpSimpleController<T>::classTypeName(), filtersAndMethods);
    }
