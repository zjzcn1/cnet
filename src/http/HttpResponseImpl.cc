...
HttpResponsePtr HttpResponse::newHttpJsonResponse(const std::string &data)
{
    auto res = std::make_shared<HttpResponseImpl>();
    res->setStatusCode(HttpResponse::k200OK);
...
void HttpResponseImpl::makeHeaderString(MsgBuffer *output) const
{
    ...
    output->append("Server: cnet/http/");
    output->append("\r\n");
}
...
void HttpResponseImpl::appendToBuffer(MsgBuffer *output) const
{
    ...
    //output Date header
    output->append("Date: ");
    output->append(HttpUtils::getHttpFullDate(cnet::Date::date()));
    output->append("\r\n\r\n");
