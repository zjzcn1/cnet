...
/* data 原数据 ndata 原数据长度 zdata 压缩后数据 nzdata 压缩后长度 */
    ...
    int HttpUtils::gzipCompress(const char *data, size_t ndata, char *zdata, size_t *nzdata) {
        z_stream c_stream;
        int err = 0;
...
/* zdata 数据 nzdata 原数据长度 data 解压后数据 ndata 解压后长度 */
    int HttpUtils::gzipDecompress(const char *zdata, size_t nzdata, char *data, size_t *ndata) {
        int err = 0;
        z_stream d_stream = {nullptr}; /* decompression stream */
