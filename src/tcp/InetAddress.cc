...
std::unordered_map<std::string, std::pair<struct in_addr, cnet::Date>> InetAddress::_dnsCache;
bool InetAddress::resolve(const std::string &hostname, InetAddress *out, size_t timeout)
{
    assert(out != NULL);
    {
        ...
        if (_dnsCache.find(hostname) != _dnsCache.end())
        {
            ...
            if (timeout == 0 || (timeout > 0 &&
                                 (addr.second.after(timeout) > cnet::Date::date())))
            {
...
#endif
    {
        ...
        out->addr_.sin_addr = *reinterpret_cast<struct in_addr *>(he->h_addr);
        {
            ...
            _dnsCache[hostname].second = cnet::Date::date();
        }
        return true;
    }
    else
